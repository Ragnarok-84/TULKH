...
INPUT_DIR = "Datasets"
SOLVER_NAME = "Evolutionary_Timetabling"
...
def fmt_param(x):
    """Đổi số thực thành chuỗi an toàn để đặt tên folder, ví dụ 0.75 -> 0p75."""
...
def make_run_name(args):
    """Tên folder chứa siêu tham số chính của lần chạy."""
    parts = [
        args.solver_name,
        ...
        f"kick{fmt_param(args.local_kick_rate)}",
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Evolutionary timetabling solver with configurable hyperparameters"
    )
    ...
    parser.add_argument("--tag", default="",
                        help="Gắn nhãn thêm cho folder thí nghiệm, ví dụ exp1 hoặc ablation.")
    ...
    parser.add_argument("--time-limit", type=float, default=None,
                        help="Ghi đè time limit cho mọi file. Nếu bỏ trống thì dùng get_time_limit(total_cc).")
    ...
    parser.add_argument("--local-kick-rate", type=float, default=0.35)
...
class EvolutionarySolver:
    def __init__(self, T, N, M, class_courses, teacher_courses, durations,
                 ...
                 crossover_rate=0.85, mutation_rate=0.75, local_kick_rate=0.35):
        self.T = T
        self.N = N
        self.M = M
        ...
        # Auto scale tham số mặc định theo kích thước bài
        n = self.total_cc
        if n <= 200:
            auto_pop_size = 40
            auto_elite = 5
        ...
        # Nếu truyền từ command line thì dùng giá trị truyền vào,
        # nếu không thì dùng auto-scale như bản gốc.
        ...
        self.local_kick_rate = local_kick_rate
    ...
    def solve(self):
        ...
        self.log(
            ...
            f"cx={self.crossover_rate} | mut={self.mutation_rate} | "
            f"kick={self.local_kick_rate} | tk={self.tournament_k}"
        )
...
def solve_one_file(filepath, verbose=True, args=None):
    ...
    solver = EvolutionarySolver(
        ...
        local_kick_rate=args.local_kick_rate if args else 0.35,
    )
